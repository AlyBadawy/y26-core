class Api::V1::WeatherEntriesController < ApplicationController
  def index
    ...
    @weather_entries = WeatherEntry.where(user: Current.user, date: start_date..end_date).order(date: :asc)
  ...
  def show
    ...
    @weather_entry = WeatherEntry.find_by(user: Current.user, date: date)
    if @weather_entry
  ...
  def upsert
    ...
    @weather_entry = WeatherEntry.find_or_initialize_by(user: Current.user, date: date)
